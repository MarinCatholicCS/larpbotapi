...
def send_request_github_email(
    ...
    html = f"""
    <div style="font-family:monospace;max-width:640px;margin:0 auto;background:#09090b;color:#e4e4e7;padding:32px;border-radius:8px;">
      <h2 style="color:#fff;margin:0 0 4px">LARPbot</h2>
      <p style="color:#71717a;margin:0 0 24px">Automated candidate verification</p>

      <div style="background:#18181b;border:1px solid #eab308;border-radius:6px;padding:20px;margin-bottom:24px">
        <p style="color:#eab308;font-weight:bold;margin:0 0 8px">{headline}</p>
        <p style="color:#a1a1aa;font-size:13px;margin:0">{detail}</p>
      </div>

      <p style="color:#d4d4d8;font-size:13px;line-height:1.7">
        To run a LARP analysis, please reply to this thread (or send a fresh email) with a
        public GitHub profile URL in the format:
      </p>
      <p style="color:#fff;font-size:14px;background:#18181b;padding:10px 14px;border-radius:6px;
                border:1px solid #27272a;margin:8px 0;font-family:monospace">
        https://github.com/&lt;username&gt;
      </p>

      <p style="color:#3f3f46;font-size:11px;margin-top:32px">Generated by LARPbot</p>
    </div>
    """
    ...
    send_gmail(
        to_email=to_email,
        subject=f"LARP Report: GitHub profile required{suffix}",
        html_body=html,
        thread_id=thread_id,
        in_reply_to=in_reply_to,
        references=references,
...
def poll_recruiting_inbox() -> dict:
    ...
    for email in emails:
        ...
        # No GitHub URL in the email — ask the sender for one.
        if not username:
            send_request_github_email(
                recruiter_email,
                headline="No GitHub profile detected",
                detail="The email did not contain a GitHub profile URL.",
                thread_id=thread_id, in_reply_to=rfc_id,
                references=refs, original_subject=orig_subject,
            )
            continue

        # Pre-validate GitHub user exists. If not, ask for a valid one.
        if not github_user_exists(username):
            send_request_github_email(
                recruiter_email,
                headline=f"GitHub profile not found: {username}",
                detail=f"The URL points to a profile that does not exist on GitHub.",
                thread_id=thread_id, in_reply_to=rfc_id,
                references=refs, original_subject=orig_subject,
            )
            continue
